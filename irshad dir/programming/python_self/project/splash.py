...
def f5():							#adst save button command
	try:
		...
		#stu_marks=None
		try:
			stu_id=int(entAddRno.get())
			if stu_id == 0:
				msg="! RollNo. coonot be 0.please Enter Correct RollNo "
		...
		except (ValueError,connector.errors.IntegrityError,Exception):
			msg="! please enter correct RollNo."
			messagebox.showerror("Error",msg)
...
# except ValueError:
	...
	# exit()
		...
		stu_name1=(entAddName.get())
		stu_name=stu_name1.capitalize()
		...
		if stu_name.isalpha():
			...
			exit()
		if len(stu_name)>2:
			msg="! Name should be atlest 2 character."
			messagebox.showerror("Error",msg)
		else:
			pass
		stu_marks=int(entAddMarks.get())
		if (stu_marks < 0 or stu_marks > 100):
			...
			args=(stu_id,stu_name,stu_marks)
			cursor.execute(sql%args)
			mydb.commit()
			messagebox.showinfo("success",'1. row inserted')
			#print('row inserted')
			entAddRno.delete(0,END)
			entAddRno.focus()
			entAddName.delete(0,END)
			entAddMarks.delete(0,END)
	except(ValueError,connector.errors.IntegrityError,Exception)as v:
		print(v)
...
def f12():
	try:
		...
		stu_name_Up=(entUpdateName.get())
		stu_Marks_Up=int(entUpdateMarks.get())
		
		if (stu_Marks_Up < 0 or stu_Marks_Up > 100):
			msg="! please Enter Correct Marks ."
			messagebox.showerror("Error",msg)
